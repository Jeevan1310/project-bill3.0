$(document).ready(function(){
    var DOMAIN = "http://localhost/bill3.0";
    ...
    $("#invoice_item").delegate(".pid", "change", function() {
		...
		$.ajax({
		  ...
		  success: function(data) {
			...
			// Remove the product from the dropdown list if the stock is 0
			if (data["product_stock"] == 0) {
			  tr.remove();
			  alert("Product Cannot be added It OUT OF STOCK");
			}
    ...
    // Remove products with stock value 0 from the dropdown list
    $.ajax({
        url: DOMAIN+"/includes/process.php",
        method: "POST",
        dataType: "json",
        ...
        success: function(data){
            $(".pid option").each(function(){
                var pid = $(this).val();
                var stock = data[pid]["product_stock"];
                if (stock == 0) {
                    $(this).remove();
                }
            });
