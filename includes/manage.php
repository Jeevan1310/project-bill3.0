...
class Manage
{
    ...
    public function manageRecordWithPagination($table, $pno)
    {
        ...
        if ($table == "categories") {
            ...
            $sql = "SELECT p.pid, p.product_name, c.category_name, b.brand_name, p.product_price, p.product_stock, p.added_date, p.p_status FROM products p JOIN brands b ON p.bid = b.bid JOIN categories c ON p.cid = c.cid " . $a["limit"];
        } else {
    ...
    {
        if ($table == "categories") {
            $pre_stmt = $this->con->prepare("SELECT `" . $pk . "` FROM `categories` WHERE `parent_cat` = ?");
